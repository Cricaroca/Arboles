...
namespace Arboles
{
    ...
    /*public class Node
    {
        ...
        private static Font font = new Font("MONOTYPE_CORSVIA", 14f * Coef);
        /// <summary>
        ...
        public Image Draw(out int center)
        {
            ...
            g.FillRectangle(Brushes.Black, new Rectangle(new Point(0, 0), resSize));
            g.DrawImage(_nodeBg, lSize.Width - _nodeBg.Width / 2 + _freeSpace.Width / 2, 0);
            var str = Value.ToString();
            g.DrawString(str, font, Brushes.Red, lSize.Width - _nodeBg.Width / 2 + _freeSpace.Width / 2 + (2 + (str.Length == 1 ? 10 : str.Length == 2 ? 5 : 0)) * Coef, _nodeBg.Height / 2f - 12 * Coef);


            center = lSize.Width + _freeSpace.Width / 2;
            var pen = new Pen(Brushes.Red, 1.2f * Coef)
            {
